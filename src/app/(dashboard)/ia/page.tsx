    ...
    <div className="animate-fade-in flex flex-col h-[calc(100vh-10rem)]">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-body text-sm mt-1">Análise e suporte inteligente para operações</p>
        </div>
        {messages.length > 0 && (
          <button onClick={() => setMessages([])}
            className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors px-3 py-1.5 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-red-200 dark:hover:border-red-800">
      ...
      <div className="flex-1 overflow-y-auto card p-4 space-y-4">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center py-12">
            <div className="w-16 h-16 icon-badge-sigma rounded-2xl flex items-center justify-center mb-4">
              <Bot className="w-8 h-8" />
            </div>
            <h3 className="text-title font-semibold mb-2">Assistente de Inteligência</h3>
            ...
            <div className="mt-6 grid grid-cols-2 gap-2 max-w-md">
              {[
                ...
                <button key={sugg} onClick={() => setInput(sugg)}
                  className="text-left text-xs text-subtle border border-gray-200 dark:border-gray-700 rounded-xl p-3 hover:border-sigma-300 dark:hover:border-sigma-700 hover:text-sigma-600 dark:hover:text-sigma-400 transition-all">
                  {sugg}
                </button>
              ))}
            </div>
          ...
          {messages.map((msg) => (
            <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className={`w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center
                ${msg.role === 'user' ? 'bg-sigma-500' : 'bg-gray-100 dark:bg-gray-800'}`}>
                {msg.role === 'user'
              ...
              <div className={`max-w-[75%] group relative ${msg.role === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap
                  ${msg.role === 'user'
                    ...
                    : 'bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-tl-sm'}`}>
                  {msg.content}
                </div>
      ...
      <div className="mt-4 flex gap-3">
        <input
          ...
          className="flex-1 px-4 py-3 input-base"
        />
