...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const finalPhotoPath = visitante?.photoPath;
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      try {
        ...
        const prodPhotoUrl = `${PROD_URL}/api/sipe/visitantes/${id}/foto`;
        ...
        if (res.ok) {
          ...
          throw new Error('Falha ao baixar foto do visitante do servidor de produção');
        }
      } catch (proxyErr) {
