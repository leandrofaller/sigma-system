...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const apenado = await prisma.sipeApenadoUnidadePrisional.findUnique({
    where: { id },
    select: {
      photoPath: true,
      nome: true,
  ...
  const finalPhotoPath = apenado?.photoPath;
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      try {
        ...
        const prodPhotoUrl = `${PROD_URL}/api/sipe/unidades-prisionais/${id}/foto`;
        ...
        if (res.ok) {
          ...
          throw new Error('Falha ao baixar foto do apenado (unidade prisional) do servidor de produção');
        }
      } catch (proxyErr) {
