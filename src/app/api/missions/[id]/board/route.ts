...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const access = await canAccessMissionBoard((await params).id, user);
  if (!access.ok) return NextResponse.json({ error: access.error }, { status: access.status });

  let lists = await prisma.boardList.findMany({
    where: { missionId: (await params).id },
  ...
  if (lists.length === 0) {
    await prisma.$transaction([
      prisma.boardList.create({ data: { missionId: (await params).id, name: 'A Fazer', position: 0, color: '#94a3b8' } }),
      prisma.boardList.create({ data: { missionId: (await params).id, name: 'Em Andamento', position: 1, color: '#f97316' } }),
      prisma.boardList.create({ data: { missionId: (await params).id, name: 'Concluído', position: 2, color: '#22c55e' } }),
    ]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  publish({ type: 'list.created', missionId: (await params).id, payload: list, actorId: user.id });
