...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const missionId = await missionIdFromChecklist((await params).id);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.boardChecklistItem.findUnique({ where: { id: (await params).id }, select: { cardId: true } });
  await prisma.boardChecklistItem.delete({ where: { id: (await params).id } });

  publish({ type: 'checklist.deleted', missionId, payload: { id: (await params).id, cardId: item?.cardId }, actorId: user.id });
