...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const missionId = await missionIdFromComment(params.id);
  if (!missionId) return NextResponse.json({ error: 'Comentário não encontrado' }, { status: 404 });

  const comment = await prisma.boardComment.findUnique({ where: { id: params.id }, select: { authorId: true, cardId: true } });
  ...
  await prisma.boardComment.delete({ where: { id: params.id } });
  publish({ type: 'comment.deleted', missionId, payload: { id: params.id, cardId: comment.cardId }, actorId: user.id });
