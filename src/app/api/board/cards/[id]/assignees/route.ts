...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await prisma.boardCardAssignee.upsert({
    where: { cardId_userId: { cardId: params.id, userId: body.userId } },
    create: { cardId: params.id, userId: body.userId },
    update: {},
  });

  const u = await prisma.user.findUnique({ where: { id: body.userId }, select: { id: true, name: true, avatar: true } });
  publish({ type: 'assignee.added', missionId, payload: { cardId: params.id, user: u }, actorId: user.id });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  publish({ type: 'assignee.removed', missionId, payload: { cardId: params.id, userId }, actorId: user.id });
