...
export async function POST(req: NextRequest) {
  ...
  const formatted = await prisma.$transaction(async (tx) => {
    ...
    const counterCfg = await tx.systemConfig.findUnique({ where: { key: 'debriefing_counter' } });
    ...
    await tx.systemConfig.upsert({
      ...
      create: { key: 'debriefing_counter', value: { next: next + 1, year } },
    });
    ...
    return `DEBRIEFING Nº ${String(next).padStart(3, '0')}/${year}/${suffix}`;
  });
