...
export function ServidorIndexingStatusFloat() {
  const { isIndexing, timedOut, progress, indexError, stopIndexing } = useServidorIndexing();
  ...
  useEffect(() => {
    if (isIndexing) {
      ...
      setShowTimeout(true);
      const timer = setTimeout(() => setShowTimeout(false), 10000);
      return () => clearTimeout(timer);
    }
  ...
  return (
    // Posicionado no centro-direito, acima do float de apenados (bottom-5 right-5)
    <div className="fixed bottom-48 right-5 z-[60] w-80 bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center gap-2">
          {timedOut ? (
            ...
            <Database className="w-4 h-4 text-teal-600 animate-pulse flex-shrink-0" />
          )}
          <span className={`text-sm font-semibold ${timedOut ? 'text-orange-600 dark:text-orange-400' : 'text-title'}`}>
            {timedOut ? 'Tempo limite atingido (170min)' : done ? 'Indexação concluída!' : 'Indexando rostos de servidores'}
          </span>
        </div>
        {isIndexing ? (
          <button
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
          <div
            className={`h-full transition-all duration-300 rounded-full ${done ? 'bg-green-500' : 'bg-gradient-to-r from-teal-500 to-teal-700'}`}
            style={{ width: `${pct}%` }}
          />
        </div>
