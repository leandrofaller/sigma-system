...
import { Upload, File, Download, Trash2, Search, Filter, Loader2, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && isAdmin && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">Importar RELINT Recebido</h3>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1.5">Agência de Origem *</label>
              <input value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })}
                placeholder="Ex: SESP/RO, DEPEN, etc."
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-sigma-400" />
            </div>
            <div>
          ...
          <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors mb-4
            ${isDragActive ? 'border-sigma-400 bg-sigma-50' : 'border-gray-200 hover:border-sigma-300 hover:bg-gray-50'}`}>
            <input {...getInputProps()} />
            {selectedFile
              ? <p className="text-sm text-sigma-600 font-medium">{selectedFile.name} ({formatFileSize(selectedFile.size)})</p>
              : <p className="text-sm text-gray-400">Arraste o arquivo aqui ou clique para selecionar<br /><span className="text-xs">PDF, DOC, DOCX, JPG, PNG</span></p>}
          </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.length === 0 && (
          <div className="col-span-3 bg-white rounded-2xl border border-gray-100 py-12 text-center">
            <File className="w-10 h-10 text-gray-200 mx-auto mb-3" />
            <p className="text-gray-400 text-sm">Nenhum arquivo importado</p>
          </div>
        )}
        {filtered.map((file, i) => (
          <motion.div key={file.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{file.source}</p>
              </div>
            </div>
