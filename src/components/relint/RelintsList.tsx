...
export function RelintsList({ relints, role, userId, userGroupId, userGroupName }: Props) {
  ...
  const handleDelete = async (id: string, status: string) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/api/relints/${id}`, { method: 'DELETE' });
      if (!res.ok) {
  ...
  const filtered = relints.filter((r) => {
    const matchSearch = !search ||
      containsNormalizedText(r.subject, search) ||
      containsNormalizedText(r.number, search);
                    ...
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {(() => {
                        const canModify = role === 'SUPER_ADMIN' || 
                                          role === 'ADMIN' || 
                                          relint.authorId === userId || 
                                          (relint.groupId === userGroupId && userGroupName !== 'NI/AIP/JI-PARANÁ');
                        return (
                          <>
                            {canModify && (
                            ...
                            {canModify && (
                              <button
                                ...
                                title={relint.status === 'DELETION_REQUESTED' ? 'Aprovar Exclusão' : 'Excluir'}
