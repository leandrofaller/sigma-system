...
export function AIPVisitantesPanel() {
  ...
  const fetchVisitantes = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/aip/visitantes')
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-teal-500 focus:border-transparent"
          />
        </div>
        <button
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        {filtered.length} registro{filtered.length !== 1 ? 's' : ''} de visitantes vinculados a apenados do AIP
      </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {paginated.map((v) => (
            <button
              ...
              className="text-left bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 hover:shadow-md hover:border-teal-300 dark:hover:border-teal-600 transition-all"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {v.parentescoVisitante && (
                    <span className="inline-block mt-1 text-xs bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 px-2 py-0.5 rounded">
                      {v.parentescoVisitante}
                    </span>
                  )}
          ...
          >
            ...
            <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                {selected.parentescoVisitante && (
                  <p className="text-sm text-teal-600 dark:text-teal-400">{selected.parentescoVisitante}</p>
                )}
              </div>
            ...
            <div className="px-6 py-4 space-y-6">
              ...
              <section>
                ...
                {selected.descricao && (
                  <div className="mt-3">
                    <p className="text-gray-500 dark:text-gray-400 text-sm">Descrição</p>
                    <p className="text-sm text-gray-900 dark:text-white mt-1 whitespace-pre-wrap">{selected.descricao}</p>
                  </div>
                )}
              </section>
