...
import {
  ...
  Folder, FolderOpen, FolderX, ChevronDown,
  Trash2, Pencil, Check, Plus, MoreVertical, Users,
} from 'lucide-react';
...
interface ArquivoFolder { id: string; name: string; color?: string | null; groupId?: string | null }

interface Props {
  files: any[];
  groups: any[];
  folders: ArquivoFolder[];
  userId: string;
  role: string;
  userGroupId: string | null;
}

export function ArquivoList({ files: initialFiles, groups, folders: initialFolders, userId, role, userGroupId }: Props) {
  ...
  // Admin: seletor de grupo para filtrar visualização
  ...
  useEffect(() => {
    if (!isAdmin) return;
    const params = selectedGroupFilter ? `?groupId=${selectedGroupFilter}` : '';
    Promise.all([
      fetch(`/api/arquivo${params}`).then((r) => r.json()),
      fetch(`/api/arquivo/folders${params}`).then((r) => r.json()),
    ]).then(([newFiles, newFolders]) => {
      setFiles(newFiles);
  ...
  const handleCreateFolder = async () => {
    ...
    const groupId = isAdmin ? (selectedGroupFilter || null) : userGroupId;
    const res = await fetch('/api/arquivo/folders', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: newFolderName.trim(), color: newFolderColor, groupId }),
    });
    if (res.ok) {
  ...
  const handleRenameFolder = async (id: string) => {
    ...
    const res = await fetch(`/api/arquivo/folders/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: renameName.trim(), color: renameColor }),
    });
    if (res.ok) {
      setFolders((prev) => prev.map((f) => f.id === id ? { ...f, name: renameName.trim(), color: renameColor } : f));
      setRenamingId(null);
    }
  };
  ...
  const handleDeleteFolder = async (id: string) => {
    ...
    const res = await fetch(`/api/arquivo/folders/${id}`, { method: 'DELETE' });
    if (res.ok) {
  ...
  const handleMoveFile = async (fileId: string, folderId: string | null) => {
    const res = await fetch(`/api/arquivo/${fileId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteFile = async (fileId: string) => {
    ...
    const res = await fetch(`/api/arquivo/${fileId}`, { method: 'DELETE' });
  ...
  // Agrupa por pasta ou fonte
  const grouped: Record<string, any[]> = {};
  for (const file of filtered) {
    const key = file.folder?.name || file.source?.trim() || 'Arquivos do Grupo';
  ...
  // Pastas visíveis: para OPERATOR, todas as pastas do grupo; para admin, as do filtro atual
  const visibleFolders = folders;

  return (
    <div className="flex gap-5 items-start">
      {/* Painel de pastas — visível para todos */}
      <div className="w-52 flex-shrink-0 card p-3 sticky top-4">
        {/* Seletor de grupo — apenas admin */}
        {isAdmin && groups.length > 0 && (
          <div className="mb-3 pb-3 border-b border-gray-100 dark:border-gray-800">
            <p className="text-xs font-semibold text-subtle uppercase tracking-wider px-2 mb-1.5">Grupo</p>
            <select
              value={selectedGroupFilter}
              onChange={(e) => setSelectedGroupFilter(e.target.value)}
              className="w-full text-xs bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1.5 text-body outline-none"
            >
        ...
        <p className="text-xs font-semibold text-subtle uppercase tracking-wider px-2 mb-2">Pastas</p>
        <nav className="space-y-0.5">
          <FolderNavItem icon={FolderOpen} label="Todos os arquivos" count={files.length} active={activeFolder === 'all'} onClick={() => setActiveFolder('all')} />
          <FolderNavItem icon={FolderX} label="Sem pasta" count={unassignedCount} active={activeFolder === 'unassigned'} onClick={() => setActiveFolder('unassigned')} />

          {visibleFolders.length > 0 && (
            <div className="pt-2 pb-1 px-2">
              <div className="border-t border-gray-100 dark:border-gray-800 pt-2">
                <p className="text-xs font-semibold text-subtle uppercase tracking-wider">Minhas pastas</p>
              </div>
            </div>
          )}
          ...
          {visibleFolders.map((folder) => (
            <div key={folder.id} className="group/folder relative">
              {renamingId === folder.id ? (
                ...
                >
                  <Folder className="w-3.5 h-3.5 flex-shrink-0" style={{ color: folder.color || '#6172f3' }} />
        ...
        {/* Nova pasta — disponível para todos */}
        {showNewFolder ? (
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="card p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-title">Adicionar ao Arquivo</h3>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                <label className="block text-xs font-medium text-subtle mb-1.5">Fonte / Origem <span className="text-gray-400">(opcional)</span></label>
                <input value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })} placeholder="Ex.: Delegacia, Relatório..." className={inputCls} />
              </div>
              {/* Seletor de grupo visível apenas para admin */}
              {isAdmin && (
                <div>
                  <label className="block text-xs font-medium text-subtle mb-1.5">Grupo destinatário</label>
              ...
              <div className="col-span-2">
                <label className="block text-xs font-medium text-subtle mb-1.5">Observações <span className="text-gray-400">(opcional)</span></label>
        ...
        {filtered.length === 0 ? (
          <div className="card py-12 text-center">
            ...
            {!isAdmin && files.length === 0 && (
              <p className="text-xs text-subtle mt-1">Seja o primeiro a enviar um arquivo para o grupo!</p>
            )}
          </div>
        ) : (
...
function ArquivoGroup({ groupKey, files, folders, isAdmin, userId, highlightId, onMove, onDelete }: {
  groupKey: string; files: any[]; folders: ArquivoFolder[]; isAdmin: boolean; userId: string;
    ...
    <div>
      <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-2 mb-3 w-full text-left group">
        <div className="w-7 h-7 bg-sigma-100 dark:bg-sigma-900/30 rounded-lg flex items-center justify-center">
          <Folder className="w-3.5 h-3.5 text-sigma-600 dark:text-sigma-400" />
        </div>
        <span className="font-semibold text-sm text-title">{groupKey}</span>
...
function ArquivoFileCard({ file, index, folders, isAdmin, userId, highlight, onMove, onDelete }: {
  file: any; index: number; folders: ArquivoFolder[]; isAdmin: boolean; userId: string;
  highlight?: boolean;
  onMove: (fileId: string, folderId: string | null) => Promise<void>;
  onDelete: (fileId: string) => Promise<void>;
}) {
  ...
  // Usuário pode gerenciar o arquivo se for admin ou o próprio uploader
  const canManage = isAdmin || file.uploadedById === userId;
  ...
  return (
    <motion.div
      ...
      className={`card p-5 hover:shadow-md transition-all relative group/card ${
        highlighted ? 'ring-2 ring-sigma-400 shadow-lg shadow-sigma-100 dark:shadow-sigma-900/30 bg-sigma-50/30 dark:bg-sigma-900/10' : ''
      }`}
    >
      {canManage && (
        <>
          ...
          {menuOpen && typeof document !== 'undefined' && createPortal(
            ...
            >
              ...
              <div className="max-h-48 overflow-y-auto">
                ...
                >
                  <FolderX className="w-3.5 h-3.5 flex-shrink-0 text-subtle" /> Sem pasta
                </button>
                {folders.map((f) => (
                  <button
                    key={f.id}
              ...
              <div className="border-t border-gray-100 dark:border-gray-800">
                ...
                >
                  ...
                  {isAdmin ? 'Excluir arquivo' : 'Excluir meu arquivo'}
                </button>
              </div>
            </div>,
            document.body
      ...
      <div className="flex items-start gap-3 mb-3">
        <div className="w-10 h-10 bg-sigma-50 dark:bg-sigma-900/20 rounded-xl flex items-center justify-center flex-shrink-0">
          <File className="w-5 h-5 text-sigma-600 dark:text-sigma-400" />
        </div>
        <div className="flex-1 min-w-0 pr-6">
          <p className="text-sm font-medium text-title truncate" title={file.title}>{file.title}</p>
          {file.source && <p className="text-xs text-subtle truncate">{file.source}</p>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        {isAdmin && file.group && (
          <span className="text-xs text-subtle bg-gray-50 dark:bg-gray-800 px-2 py-0.5 rounded-full border border-gray-200 dark:border-gray-700 flex items-center gap-1">
            <Users className="w-2.5 h-2.5" /> {file.group.name}
          </span>
        )}
      </div>
