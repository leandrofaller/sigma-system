...
import {
  ...
  Gauge, CheckCircle2, Clock, AlertCircle, XCircle, Search, Loader2, X
} from 'lucide-react';
...
export function MissionReport({ missions, groups, users, isAdmin, currentUserId }: Props) {
  ...
  const handleAssociateDebriefing = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setLocalMissions(ms => ms.map(m => m.id === data.id ? { ...m, debriefing: data.debriefing } : m));
  ...
  }, [localMissions, search, status, groupId, userId, from, to, isAdmin]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <SummaryCard
          ...
          value={stats.totalKm.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
          accent="green"
        />
        <SummaryCard
          icon={MapPin}
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {(() => {
                              ...
                              if (isAdmin || isPart) {
                                return (
                                  <button
                                    onClick={() => setAssociateDebriefing({ open: true, missionId: m.id, debriefingNumber: '' })}
                    ...
                    <td className="px-4 py-3 text-xs text-right font-mono whitespace-nowrap">
                      {km != null ? (
                        ...
                        <span className="text-subtle">
                          {m.startKm != null ? `${m.startKm.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} → ?` : '—'}
                        </span>
                      )}
                    </td>
      ...
      {associateDebriefing.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setAssociateDebriefing({ open: false, missionId: '', debriefingNumber: '' })}>
          <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-md shadow-2xl relative p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <h3 className="text-base font-bold text-title flex items-center gap-2">
                📄 Relacionar Debriefing
              </h3>
              <button
                onClick={() => setAssociateDebriefing({ open: false, missionId: '', debriefingNumber: '' })}
            ...
            <p className="text-xs text-body leading-relaxed">
              Informe o número do Debriefing que deseja relacionar a esta viagem. A associação será realizada de forma instantânea.
            </p>
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full input-base px-4 py-2.5 text-xs"
              />
            </div>

            <div className="flex gap-2 pt-2">
              <button
                onClick={handleAssociateDebriefing}
                disabled={loading}
                className="flex-1 bg-sigma-600 hover:bg-sigma-700 text-white font-bold py-2.5 rounded-2xl text-xs transition-all active:scale-95 shadow-md shadow-sigma-600/10 flex items-center justify-center gap-1.5"
              >
