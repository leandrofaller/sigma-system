...
async function testFichaGeralPost() {
  console.log('Iniciando teste de POST real na Ficha Geral...')
  ...
  try {
    ...
    // 2. Faz requisição POST via fetch na página enviando os campos corretos
    ...
    const postResult = await page.evaluate(async ({ url, sipeId, token }) => {
      try {
        ...
        bodyParams.append('listar[]', 'DP') // Dados Pessoais
    ...
    if (postResult.error) {
      ...
      fs.writeFileSync('scratch/ficha-geral-post-real.html', postResult.html)

      const isWhoops = postResult.html.includes('Whoops') || postResult.html.includes('something went wrong')
      console.log(`Contém Whoops? ${isWhoops}`)
      
      const isHome = postResult.html.includes('PÁGINA PRINCIPAL')
      console.log(`Redirecionou para a Home? ${isHome}`)

      if (!isWhoops && !isHome && postResult.textLength > 1000) {
        ...
        // Vamos extrair as tabelas do HTML de Ficha Geral retornado
        // E logar quais cabeçalhos e linhas foram encontrados
        const tables = await page.evaluate((htmlString) => {
          const parser = new DOMParser()
          const doc = parser.parseFromString(htmlString, 'text/html')
          const foundTables = doc.querySelectorAll('table')
          
          return Array.from(foundTables).map((table, idx) => {
            const titleEl = table.previousElementSibling
            const titleText = titleEl ? titleEl.textContent?.trim() : ''

            const headers = Array.from(table.querySelectorAll('th, thead td, tr th')).map(el => el.textContent?.trim() || '')
        ...
        console.log('Tabelas encontradas na Ficha Geral carregada:')
        console.log(JSON.stringify(tables, null, 2))
      } else if (isHome) {
        // Extrai a mensagem de alerta da Home
        const alertMatch = postResult.html.match(/alert-warning[^>]*>([\s\S]*?)<\/div>/i)
        if (alertMatch) {
          console.log('Alerta na Home:', alertMatch[1].replace(/<[^>]*>/g, '').trim())
        }
      }
    }
