...
async function main() {
  ...
  try {
    ...
    console.log(`Navegando para o index de apenados...`)
    const response = await page.goto(`${SIPE_URL}/apenados/index`, { waitUntil: 'domcontentloaded', timeout: 45000 })
    
    console.log('URL final:', page.url())
    console.log('Status da resposta:', response?.status())
    
    await page.waitForSelector('table', { timeout: 15000 }).catch(() => console.log('Tabela não encontrada, mas salvando HTML mesmo assim.'))
    
    const html = await page.content()
    writeFileSync('scratch/index-page.html', html)
    console.log('HTML salvo em scratch/index-page.html')
